...
const generate1D = () => {
  ...
  return Array.from({ length: 366 }, (_, i) => {
    years.forEach((y) => {
      values[y] += Math.floor(Math.random() * 7 - 3);
      values[y] = Math.max(20, Math.min(160, values[y]));
    });
...
const generate1W = () => {
  ...
  return Array.from({ length: 52 }, (_, i) => {
    years.forEach((y) => {
      values[y] += Math.floor(Math.random() * 9 - 4);
      values[y] = Math.max(30, Math.min(180, values[y]));
    });
...
const generate1M = () => {
  ...
  return [
    ...
    years.forEach((y) => {
      values[y] += Math.floor(Math.random() * 5 - 2);
      values[y] = Math.max(60, Math.min(140, values[y]));
    });
...
const generate3M = () => {
  ...
  return ["January", "April", "July", "October"].map((month) => {
    years.forEach((y) => {
      values[y] += Math.floor(Math.random() * 3 - 1);
      values[y] = Math.max(80, Math.min(130, values[y]));
    });
